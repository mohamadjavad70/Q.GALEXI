...
import { analyzeMemory, getEcosystemHealth } from './memoryIntelligence.js';
import {
  guardianMiddleware,
  analyzeWithGuardian,
  getGuardianScores,
...
// Analyzes every /api/* request via the Guardian FastAPI service.
// Fail-open: if Guardian is offline, traffic passes through unblocked.
...
// GET /api/memory/insights — Memory Intelligence
app.get('/api/memory/insights', (_req: Request, res: Response) => {
  res.json(analyzeMemory(globalMemory));
});

// GET /api/memory/health — وضعیت سلامت اکوسیستم
app.get('/api/memory/health', (_req: Request, res: Response) => {
  res.json(getEcosystemHealth(globalMemory));
});
...
// ─── Guardian Intelligence Endpoints ─────────────────────────────────────────

/** سلامت Guardian */
app.get('/api/guardian/health', async (_req: Request, res: Response) => {
  const alive = await isGuardianAlive();
  res.json({ guardian: alive ? 'online' : 'offline' });
});

/** همه امتیازهای ریسک IP */
app.get('/api/guardian/scores', async (_req: Request, res: Response) => {
  const scores = await getGuardianScores();
  res.json({ scores: scores ?? {} });
});

/** لاگ‌های اخیر (با فیلتر اختیاری ip) */
app.get('/api/guardian/logs', async (req: Request, res: Response) => {
  const ip    = typeof req.query.ip    === 'string' ? req.query.ip    : undefined;
  const limit = typeof req.query.limit === 'string' ? Number(req.query.limit) : 50;
  const logs  = await getGuardianLogs(ip, limit);
  res.json({ logs: logs ?? [] });
});

/** تحلیل دستی یک IP/request */
app.post('/api/guardian/analyze', async (req: Request, res: Response) => {
  const { ip, request: reqBody, timestamp } = req.body as {
    ip?: string; request?: string; timestamp?: string;
  };
  if (!ip || !reqBody) {
    res.status(400).json({ error: 'ip و request اجباری هستند' });
    return;
  }
  const result = await analyzeWithGuardian({
    ip,
    request: reqBody,
    timestamp: timestamp ?? new Date().toISOString(),
  });
  if (!result) {
    res.status(503).json({ error: 'Guardian آفلاین است' });
    return;
  }
  res.json(result);
});
...
app.listen(PORT, () => {
  ...
  isGuardianAlive().then((alive) => {
    if (alive) {
      console.log('[Guardian] ✔ connected — security layer active');
    } else {
      console.warn('[Guardian] ✘ offline — fail-open mode (requests pass through)');
    }
  });
});
